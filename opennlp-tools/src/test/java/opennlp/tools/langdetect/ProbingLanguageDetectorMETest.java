...
public class ProbingLanguageDetectorMETest {
  ...
  public void testPredictLanguages() {
    LanguageDetector ld = new ProbingLanguageDetectorME(this.model);
    for (int i = 0; i < 10000; i += 1000) {
      StringBuilder sb = new StringBuilder();
      for (int j = 0; j <= i; j++) {
        sb.append("estava em uma marcenaria na Rua Bruno ");
      }
      Language[] languages = ld.predictLanguages(sb.toString());
  ...
  public static LanguageDetectorSampleStream createSampleStream() throws IOException {
    ...
    ResourceAsStreamFactory streamFactory = new ResourceAsStreamFactory(
        ProbingLanguageDetectorMETest.class, "/opennlp/tools/doccat/DoccatSample.txt");
